...
def update_worksheet(data, worksheet):
    ...
    print (f"{worksheet.title()} worksheet updated successfully. The new {worksheet} data are: {data} \n")
    

def calculate_surplus_data(sales_row):
    """
    Compare sales with stock and calculate thje surplus for each item type.
...
def main():
    ...
    # Show user the next stock market
    stock_values = get_stock_values(stock_data)
    print("--- Make the following numbers of sandwiches for next market: ---\n")
    for sandwich, amount in stock_values.items():
        print(f"{sandwich}: {amount}")
